...
export default function AuthPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (isAccountLocked) {
      setErrorMessage('System locked. Too many failed attempts.');
      setIsLoading(false);
      return;
    }
    ...
    if (result.success) {
      ...
      if (newAttempts >= MAX_ATTEMPTS) {
        ...
        setErrorMessage('System closed due to too many failed attempts!');
        setTimeout(() => {
          setErrorMessage('');
        }, 3000);
      } else {
            ...
            {loginAttempts > 0 && (
              <div
                className={`mb-4 p-3 rounded-lg text-sm ${
                  loginAttempts >= MAX_ATTEMPTS ? 'attempts-critical' : 'attempts-warning'
                }`}
              >
                <div className="flex items-center">
                  ...
                  <span>
                    {loginAttempts === 1 && `1 failed attempt. ${MAX_ATTEMPTS - loginAttempts} attempts remaining.`}
                    {loginAttempts === 2 && `2 failed attempts. Last attempt before system closure!`}
                    {loginAttempts >= MAX_ATTEMPTS && 'Too many failed attempts! System closing...'}
                  </span>
                </div>
              </div>
            )}
            ...
            <form onSubmit={handleSignup}>
              ...
              <div className="mb-4">
                ...
                <div className="mt-2 space-y-1 text-xs">
                  <div className={`password-requirement flex items-center ${passwordValidation.hasMinLength ? 'valid' : 'invalid'}`}>
                  ...
                  <div className={`password-requirement flex items-center ${passwordValidation.hasUpperCase ? 'valid' : 'invalid'}`}>
                    <svg className="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <div className={`password-requirement flex items-center ${passwordValidation.hasNumber ? 'valid' : 'invalid'}`}>
                    <svg className="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <div className={`password-requirement flex items-center ${passwordValidation.hasSpecialChar ? 'valid' : 'invalid'}`}>
                    <svg className="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <button
                ...
                className={`w-full bg-blue-600 text-white py-3 px-4 rounded-lg font-medium hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition flex items-center justify-center ${
                  isSignupDisabled || isLoading ? 'opacity-70 cursor-not-allowed' : ''
                }`}
              >
                {isLoading ? (
                  <>
